...
async function apiCalendar(env,token){const cfg=await getConfig(env);const now=new Date(),end=new Date(Date.now()+60*864e5);const u=new URL("https://www.googleapis.com/calendar/v3/calendars/primary/events");u.searchParams.set("timeMin",now.toISOString());u.searchParams.set("timeMax",end.toISOString());u.searchParams.set("singleEvents","true");u.searchParams.set("orderBy","startTime");u.searchParams.set("maxResults","100");const d=await gfetch(u.toString(),token).then(r=>r.json());return json({events:(d.items||[]).map(e=>({summary:e.summary||"(no title)",location:e.location||"",eventType:e.eventType||"default",start:e.start||{},end:e.end||{},status:e.status}))});}
async function apiGmail(env,token){const list=await gfetch("https://hgmail.googleapis.com/gmail/v1/users/me/messages?q=in%3Ainbox&maxResults=12",token).then(r=>r.json());const ids=(list.messages||[]).map(m=>m.id);const msgs=await Promise.all(ids.map(id=>gfetch("https://gmail.googleapis.com/gmail/v1/users/me/messages/"+id+"?format=metadata&metadataHeaders=From&metadataHeaders=Subject&metadataHeaders=Date",token).then(r=>r.json())));const messages=msgs.map(m=>{const hs=((m.payload&&m.payload.headers)||[]).reduce((a,h)=>{a[h.name.toLowerCase()]=h.value;return a;},{});return {sender:hs.from||"",subject:hs.subject||"(no subject)",date:(d=>{const t=d?new Date(d):null;return t&&!isNaN(t.getTime())?t.toISOString():null;})(hs.date),snippet:m.snippet||"",labelIds:m.labelIds||[],threadId:m.threadId||""};});let draftThreads=[];try{const dl=await gfetch("https://gmail.googleapis.com/gmail/v1/users/me/drafts?maxResults=50",token).then(r=>r.json());draftThreads=(dl.drafts||[]).map(d=>d.message&&d.message.threadId).filter(Boolean);}catch(e){}return json({messages,draftThreads});}
...
if(p==="/api/gmail")return await apiGmail(env,token);
